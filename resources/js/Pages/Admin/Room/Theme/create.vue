<template>
    <div class="flex -mt-28 mx-24 gap-8 pb-64">
        <form class="glass shadow-lg rounded-lg mb-5 p-4" @submit.prevent="submitForm">
            ...
            <p v-if="formData.errors && formData.errors.frame" class="text-error">
                You must select a frame
            </p>
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>
                            <label>
                                <input
                                    ...
                                    class="radio radio-primary shadow-lg shadow-primary/40"
                                />
                            </label>
                        </th>
                        <td>
            ...
            <div class="mockup-phone">
                ...
                <div class="display">
                    <div class="artboard phone-1 relative">
                        <div class="room" :style="`background-image: url('${background_preview}');`">
                            <div>
                                ...
                                <div class="grid grid-cols-5 text-center text-xs">
                                    ...
                                    <div v-for="n in user.room.allowed_seats" class="flex-col-middle text-[--theme-text-color]" style="height: 82px;">
                                        <Avatar
                                            :frameSrc="themeFrame ? themeFrame.src_animated : ''"
...
let background_preview = ref('');
let thumbnail_preview = ref('');
...
watch(formData, async (newFormData, oldFormData) => {
    disableForm =
        ...
        newFormData.price !== '' &&
        newFormData.color !== '' &&
        newFormData.text_color !== '' &&
        newFormData.icons_color !== '' &&
        newFormData.background !== '' &&
        newFormData.thumbnail !== '' &&
        newFormData.frame !== null;

    themeFrame = props.frames.find(obj => obj.id === newFormData.frame);
})
const submitForm = () => {
    formData.post(route('admin.room.theme.store'));
};
...
const user = ref({
    ...
    room: {
        ...
        theme: {
            ...
            iconColor: formData.icons_color,
            frame: {
...
/* Room styles */
.room {
    --theme-color: v-bind(formData.color);
    --theme-text-color: v-bind(formData.text_color);
    --theme-icon-color: v-bind(formData.icons_color);
    --theme-shadow-color: rgb(from v-bind(formData.color) r g b / 0.3);
    --avatar-border: v-bind(themeFrame.bdr_box);
    --avatar-size:  v-bind(themeFrame.size);
...
.room_head_block {
    ...
    font-weight: bold;
    background-image: linear-gradient(to top left, var(--theme-color), var(--theme-shadow-color));
    box-shadow: 0 10px 15px rgba(0, 0, 0, 0.3), var(--theme-shadow-color) 0 15px 30px -10px;
