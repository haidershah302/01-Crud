        ...
        <div class="room" :style="`background-image: url('${user.room.theme.background_image}');`">
            <div>
                <div class="flex gap-2 px-2">
                    ...
                    <div class="room_head_block">
                        ...
                        <div>
                            <Avatar
                                ...
                                class="mx-auto"
                            />
                            <p class="text-xs font-bold px-2 -mt-2 truncate">{{ user.name }}</p>
                        </div>

                        <div class="flex-col-middle text-sm">
                            <h1>{{ user.room.name }}</h1>

                            <p>ID: {{ user.room.id }}</p>
                        </div>

                        <div class="flex-col-middle book-mark-button flex-shrink">
                    ...
                    <div class="flex-col-middle ml-auto">
...
const user = {
    ...
    room: {
        ...
        allowed_seats: 15,
        theme: {
            background_image: assetDir + '/site_assets/backgrounds/15.webp',
            frame: {
...
.room {
    ...
    overflow: hidden;
    height: 676px;
    @apply flex flex-col justify-between pt-7 pb-5 max-h-screen overflow-hidden;
}

.room_head_block {
    @apply flex justify-between rounded-box font-bold overflow-hidden
    bg-gradient-to-tl shadow-xl flex-grow
    from-[--theme-color] shadow-[--theme-shadow-color] text-[--theme-text-color];
}

.btn-nav {
    @apply btn btn-circle border-4 btn-outline shadow-lg;
}

.share-button {
    @apply border-green-500 shadow-green-500 text-green-500;
}

.close-button {
    @apply border-red-500 shadow-red-500 text-red-500;
}

.book-mark-button {
    @apply border-rose-500 text-rose-500 drop-shadow-lg;
}
