...
const themes = ['FlyLiveUi', 'FlyLiveUiDark'];
...
onMounted(() => {
    const initialTheme = window.matchMedia('(prefers-color-scheme: FlyLiveUiDark)').matches ? 'FlyLiveUiDark' : 'FlyLiveUi';
    switchTheme(initialTheme);

    window.matchMedia('(prefers-color-scheme: FlyLiveUiDark)').addEventListener('change', (e) => {
        switchTheme(e.matches ? 'FlyLiveUiDark' : 'FlyLiveUi');
    });
});
</script>
